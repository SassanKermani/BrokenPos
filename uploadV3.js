...
//read names of directory and files
let fsReaddir = (spot)=>{
...
let populateFoldorsFromSection = ()=>{
	populateFoldorsFromSectionSpecial();
	fsReaddir(`./db`).then((listOfFiles)=>{
		populateFoldorsFromSectionNormal(listOfFiles)
	})
}

let populateFoldorsFromSectionSpecial = ()=>{
	
	//import
	let buttonImport = document.createElement(`button`);
	buttonImport.innerHTML = `Import`;
	buttonImport.classList.add(`block`);
	buttonImport.addEventListener(`click`, ()=>{
		//DO THIS 
	});
	document.getElementById(`selectFromFoldorsSpecal`).appendChild(buttonImport);

	//all
	let buttonAll = document.createElement(`button`);
	button.innerHTML = `All Videos`;
	buttonAll.classList.add(`block`);
...
let populateFoldorsFromSectionNormal = (listOfFiles)=>{
